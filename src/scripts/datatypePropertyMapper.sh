    ...
    for j in "${!domainDataPropLink[@]}"; do 
        if [[ ${domainDataPropLink[$j]} = ${classlink[$i]} ]]; then 
            ...
            if [[ ${dataProp[$j]} = 'total' ]]; then 
                echo '<div class="form-group col-lg-4 col-md-4"><label class="control-label" for="'${class[$i],,}''${dataProp[$j],,}'">{_ '${dataProp[$j]}' _}</label><div><input class="form-control" id="'${class[$i],,}''${dataProp[$j],,}'" type="'$type'" name="'${class[$i],,}''${dataProp[$j],,}'" value="{{ r.'${class[$i],,}''${dataProp[$j],,}' }}"  disabled/></div></div>{% javascript %}document.getElementById("'${class[$i],,}''${dataProp[$j],,}'").value = {{m.abs.totalDonation[{query id=id}]}};{% endjavascript %}' >> modules/mod_admin/templates/_admin_edit_content_main; 
                mainuser=$mainuser'<p class="'${class[$i],,}''${dataProp[$j],,}'" id="'${class[$i],,}''${dataProp[$j],,}'"><b>{_ '${dataProp[$j]}' _}</b> : </p> {% javascript %} var total = {{m.abs.totalDonation[{query id=id}]}}; if (total == 0) { document.getElementById("'${class[$i],,}''${dataProp[$j],,}'").className += " hidden"; } else { document.getElementById("'${class[$i],,}''${dataProp[$j],,}'").innerHTML += total;}{% endjavascript %}';
            else 
